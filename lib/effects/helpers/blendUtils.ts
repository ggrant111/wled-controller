/**
 * Blend mode utilities for combining effect layers
 */

import { BlendMode, RGBColor } from './colorUtils';

/**
 * Blend two RGB colors using the specified blend mode and opacity
 */
export function blendColors(
  ...
  // If opacity is 0, return base color
  if (alpha === 0) {
    return baseColor;
  }
  ...
  switch (blendMode) {
    ...
    case 'screen':
      // Inverse multiply (brighten, blend)
      const screen = {
    ...
    case 'hard-light':
      // Strong light effect with high contrast
      const hardLight = {
    ...
    case 'max':
      // Take maximum of each color channel
      blended = {
    ...
    case 'min':
      // Take minimum of each color channel
      blended = {
    ...
    case 'replace':
      // Replace pixels where layer has color (non-black)
...
/**
 * Blend multiple effect frames together
 * All frames must be the same length (ledCount * 3 bytes)
 */
export function blendFrames(
  baseFrame: Buffer,
  layerFrames: Array<{ frame: Buffer; blendMode: BlendMode; opacity: number }>
): Buffer {
  ...
  // Validate buffer lengths - all frames must match
  if (baseFrame.length % 3 !== 0) {
    console.warn('Base frame length is not a multiple of 3 (invalid RGB data)');
    return baseFrame;
  }
  
  for (const layer of layerFrames) {
    if (layer.frame.length !== baseFrame.length) {
      console.warn(`Layer frame length mismatch: expected ${baseFrame.length}, got ${layer.frame.length}`);
      return baseFrame; // Return unblended frame if mismatch
    }
    if (layer.frame.length % 3 !== 0) {
      console.warn('Layer frame length is not a multiple of 3 (invalid RGB data)');
      return baseFrame;
    }
  }
  
  // Create a copy of the base frame to blend into
  const result = Buffer.from(baseFrame);
  
  // Blend each layer on top of the base
  for (let i = 0; i < ledCount; i++) {
    ...
    // Apply each layer on top of the current result (blend sequentially)
    for (const layer of layerFrames) {
      const layerColor = {
      ...
      // Blend this layer on top of the current accumulated color
      currentColor = blendColors(currentColor, layerColor, layer.blendMode, layer.opacity);
    }
    
    // Write blended color back to result buffer
